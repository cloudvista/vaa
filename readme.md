...
### Summary
VistA Application Analysis (VAA) is a first-ever comprehensive capture and analysis of VA's clinical workflows. VAA is based on the capture of all traffic generated by all users of all VA point-of-care applications in operation at VA medical centers. All workflows are identifiable by specific user, specialty, facility all the way to each invividual transaction.

VAA captures and analyzes VA's workflows to improve the use of current clients, which improves clinical efficiency, expands access to care, and reduces costs.  Comprehensive workflow capture also enables all current workflows to  be migrated to new clients and systems -  without a loss of clinical efficiency.

* This traffic-based approach to capturing workflow is a proven, emprical, data-driven approach. It is not notional or speculative, or based on manual human observation. VAA comprehensively captures all the traffic and identifies all users and usage of all care processes - down to each individual transaction.
