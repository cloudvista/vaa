...
### Summary
VistA Application Analysis (VAA) is a first-ever comprehensive capture and analysis of VA's clinical workflows. VAA is based on the capture of all traffic generated by all users of all VA care applications in operation at VA medical centers. All workflows are identifiable by specific user, specialty, facility all the way to each invividual transaction.

* VAA captures and analyzes VA's clinmical workflows to improve the use of VA's care applications - improving clinical efficiency, expanding access to care, and reducing costs.
* Comprehensive workflow capture also enables all current workflows to  be migrated to new clients and systems -  without a loss of clinical efficiency.
* VAA's traffic-based approach to capturing workflow is a proven, emprical, data-driven approach. It is not notional or speculative, or based on manual human observation. VAA comprehensively captures all the traffic from all users of all care applications, and identifies each compoent of every care processes - down to each individual transaction.
* The captured VA clinical workflows incorporate 35 years of accumulated institutional knowlege of VA- and veteran-specific care, which is essential to preserve to maintain efficiency and veteran-centric care as VA migrates to new healthcare information systems.
