...
#### Workflow Capture
All VistA Application workflows (RPC traffic) of cloud-based VistA is streamed to a cloud storage using the built-in traffic mirroring service in the AWS Cloud.
...
1. __Filtered Analysis__: Simple/filtered analysis of types and volumes of use of all clients of a given VistA. This provides *screen metrics* to be used in the VistA application-specific analysis.
2. __Sequence Analysis__: Identify and classify the longest common sequences (LCS) of RPCs generated by VistA Applications. This correlates to the *actual workflow* of end-users of a VistA Application.
3. ___Workflow Analysis__: The Vista Documentation Library (VDL) contains the contains User Guides for VistA Applications, which describe the *documented workflow* of a given VistA Application. This documentation and screenshots will be parsed and classified in a standardized form to permit analysis.
4. __Workflow Correlation Analysis__ - Correlate the (1) Actual screens generated by live users of a VistA Applicaiton (2) Documented screens based on the VistA application user guides in the VistA Documenation Library  (3) Actual workflow - based on longest common sequence (LCS) analyss of known RPCs an their fuction.
